...
def new_func():
    ...
    email_username = 'echomail77@gmail.com'
    email_password = 'qqlfufzfcwhljnmb'
    s.starttls()
    s.login(email_username, email_password)
    attachment_dir = 'C:/Users/Munazil/OneDrive/Documents/project/Voice-Based-email-for-Visually-Challenged-people-master/voice_based_email/mysite/homepage/attachments'
    if not os.path.exists(attachment_dir):
        os.makedirs(attachment_dir)
        print(f"Directory created: {attachment_dir}")
    else:
        print(f"Directory already exists: {attachment_dir}")
    return s,attachment_dir

s, attachment_dir = new_func()
...
def texttospeech(text, filename):
    ...
    if os.path.exists(filename):
        playsound(filename)  # Ensure this line is correct
        os.remove(filename)
    else:
...
def login_view(request):
    ...
    if request.method == 'POST':
        text1 = "Welcome to our Voice Based Email."
        texttospeech(text1, f'{file}{i}')
        i += 1

        flag = True
        i = 0  # Assuming 'i' starts at 0
        while flag:
            texttospeech("Enter your Email", f'{file}{i}')
            i += 1
            addr = speechtotext(10)  # Capture email address via speech
    
            if addr != 'N':
        # Confirm the email with the user
               texttospeech(f"You meant {addr}. Say 'confirm' to proceed or 'no' to re-enter.", f'{file}{i}')
               i += 1
               say = speechtotext(4).lower().strip()  # Convert response to lowercase
               print(f"Captured response: '{say}'")  # Debugging output to see actual response

               if say in ['yes', 'confirm', 'yeah', 'yea']:  # Allow common variations
                   flag = False
                   texttospeech("Email confirmed. Thank you!", f'{file}{i}')
               elif say == 'no':
                   texttospeech("Let's try again.", f'{file}{i}')
               else:
                   texttospeech("Unrecognized response. Let's try again.", f'{file}{i}')
            else:
                texttospeech("Could not understand your input. Please try again.", f'{file}{i}')
                i += 1
        ...
        while flag:
            ...
            i += 1
            passwrd = speechtotext(10).strip()
            
            if passwrd != 'N':
                texttospeech("You meant " + passwrd + " say confirm to proceed or no to enter again", f'{file}{i}')
                i += 1
                say = speechtotext(4).strip().lower()

                 # Debugging outputs for development purposes (remove in production)
                print(f"Recognized password: {passwrd}")  # Log the password (masked in real use)
                print(f"Confirmation response: {say}")   # Log the response
                
                if say in ['yes', 'yeah', 'yep', 'confirm']:  # Check for variations of 'yes'
                    flag = False
                    texttospeech("Password confirmed. Thank you!", f'{file}{i}')
                elif say in ['no', 'nah', 'nope']:  # Check for variations of 'no'
                    texttospeech("Let's try again.", f'{file}{i}')
                else:
                    texttospeech("Unrecognized response. Please say 'yes' or 'no'.", f'{file}{i}')
            else:
        ...
        try:
            ...
            texttospeech("User does not exist. Please check the email address.", f'{file}{i}')
            i += 1
            return JsonResponse({'result': 'failure', 'error': 'User not found'})
        except Exception as e:
            texttospeech("An unexpected error occurred. Please try again later.", f'{file}{i}')
            i += 1
            print(f"Error during login: {e}")  # Log the specific error for debugging
...
def compose_view(request):
    ...
    if request.method == 'POST':
        ...
        while flag1:
            while flag:
                ...
                if to != 'N':
                    ...
                    texttospeech("You meant " + to + " say confirm to proceed or no to enter again", f'{file}{i}')
                    i +=1
                    say = speechtotext(5).lower()
                    if say == 'confirm':
                        toaddr.append(to)
                        flag = False
                else:
                    texttospeech("could not understand what you meant", f'{file}{i}')
                    i +=1
            texttospeech("Do you want to enter more recipients ?  Say confirm or no.", f'{file}{i}')
            i +=1
            say1 = speechtotext(4).lower()
            if say1 == 'no':
                flag1= False
            else:
                flag = True
        ...
        texttospeech("any attachment? say confirm or no", f'{file}{i}')
        i +=1
        x = speechtotext(4)
        x = x.lower()
        if x == 'confirm':
            ...
            if say == 'confirm':
                ...
                try:
                    ...
                    texttospeech("Error attaching the file. Please check the filename and try again.", f'{file}{i}')
                    i += 1
                    print(e)
                    return JsonResponse({'result': 'failure'})
        elif x == 'no':
            texttospeech("No attachments will be added.", f'{file}{i}')
            i += 1
        try:
...
def search_specific_mail(folder, key, value, foldername):
    ...
    # Filter mails from the database where recipient matches the key-value pair
    # Assuming the `Mail` model has fields like recipient, subject, and status (folder)
    if key == "recipient":
        mails = Mail.objects.filter(recipient=value, status=folder)  # You are filtering by recipient and folder
    elif key == "subject":
        mails = Mail.objects.filter(subject__icontains=value, status=folder)  # Searching by subject
    else:
        # Handle other types of search criteria here
        mails = Mail.objects.filter(status=folder)

    # Check if there are any mails matching the search criteria
    if mails.exists():
        texttospeech(f"There are {len(mails)} emails with this search criteria.", f'{file}{i}')
        i += 1
        
        # Read the mails (this could be a function that processes the list of emails)
        read_mails(mails, foldername)
    else:
        texttospeech("There are no emails with this search criteria.", f'{file}{i}')
        i += 1
        ...
        while(flag):
            ...
            if act == 'unread' or act=='android' or act=='hundred':
                ...
                while True:
                    ...
                    texttospeech("You meant " + emailid + " say confirm to proceed or no to enter again", f'{file}{i}')
                    i +=1
                    yn = speechtotext(5)
                    yn = yn.lower()
                    if yn == 'confirm':
